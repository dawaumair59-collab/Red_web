...
function PaymentOption({ value, selected, icon, label, description, onClick }: {
  value: PaymentMethod; selected: boolean; icon: React.ReactNode;
...
export function CartDrawer({ open, onClose, tableId }: CartDrawerProps) {
  const {
    ...
    subtotal, tax, total, paymentMethod, setPaymentMethod, updateNote,
  } = useCart();
  ...
  const { recommendations } = useCartRecommendations(cartItemIds);

  const goTo = (next: Step, dir: 1 | -1) => { setDirection(dir); setStep(next); };

  const handlePlaceOrder = () => {
    if (items.length === 0) return;
    const notesText = [orderNotes, ...items.filter((i) => i.note).map((i) => `${i.name}: ${i.note}`)]
      .filter(Boolean).join("\n") || undefined;

    createOrder.mutate(
      { data: { tableId, customerName: customerName.trim() || undefined, specialRequests: notesText, paymentMethod: paymentMethod as "online" | "cod", items: items.map((i) => ({ menuItemId: i.menuItemId, quantity: i.quantity })) } },
      {
        onSuccess: (order) => {
          clearCart(); onClose(); setStep("cart"); setCustomerName(""); setOrderNotes("");
      ...
      <SheetContent side="right" className="w-full sm:max-w-md flex flex-col p-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden relative">
          <AnimatePresence initial={false} custom={direction} mode="wait">
            {step === "cart" ? (
              <motion.div key="cart" custom={direction} variants={slideVariants}
                initial={direction > 0 ? "enterLeft" : "enterRight"} animate="center"
              ...
              >
                {isEmpty ? (
                  <div className="flex-1 flex flex-col items-center justify-center gap-4 px-8 text-center">
                    ...
                    <Button variant="outline" onClick={handleClose} className="mt-2">Browse Menu</Button>
                  </div>
                ) : (
                  <>
                    <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
                      <AnimatePresence>
                        {items.map((item) => (
                          ...
                          >
                            <div className="bg-card border border-border rounded-xl p-3 space-y-2" data-testid={`row-cart-${item.menuItemId}`}>
                              <div className="flex items-center gap-3">
                                {item.imageUrl ? (
                                  <img src={item.imageUrl} alt={item.name} className="h-12 w-12 rounded-lg object-cover flex-shrink-0" loading="lazy" />
                                ) : (
                                  <div className="h-12 w-12 rounded-lg bg-muted flex items-center justify-center flex-shrink-0 text-xl">🍽</div>
                                )}
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-xs text-primary font-bold mt-0.5">
                                    ...
                                    {item.quantity > 1 && (
                                      <span className="text-muted-foreground font-normal ml-1">(₹{item.price.toFixed(0)} × {item.quantity})</span>
                                    )}
                                  </p>
                                </div>
                                <div className="flex items-center gap-1 bg-muted rounded-lg p-0.5">
                                  <button className="h-7 w-7 flex items-center justify-center rounded-md hover:bg-background transition-colors text-foreground"
                                    onClick={() => updateQuantity(item.menuItemId, item.quantity - 1)}
                                    data-testid={`button-cart-decrease-${item.menuItemId}`}>
                                    {item.quantity === 1 ? <Trash2 className="h-3 w-3 text-destructive" /> : <Minus className="h-3 w-3" />}
                                  </button>
                                  <span className="w-5 text-center text-sm font-bold" data-testid={`text-cart-qty-${item.menuItemId}`}>{item.quantity}</span>
                                  <button className="h-7 w-7 flex items-center justify-center rounded-md hover:bg-background transition-colors"
                                    onClick={() => updateQuantity(item.menuItemId, item.quantity + 1)}
                              ...
                              <div>
                                {expandedNote === item.menuItemId ? (
                                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex gap-2">
                                    <input type="text" value={item.note ?? ""} onChange={(e) => updateNote(item.menuItemId, e.target.value)}
                                      ...
                                      autoFocus data-testid={`input-item-note-${item.menuItemId}`} />
                                    <button className="text-xs text-muted-foreground hover:text-foreground px-1" onClick={() => setExpandedNote(null)}>Done</button>
                                  </motion.div>
                                ) : (
                                  <button className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors"
                                    ...
                                    {item.note ? <span className="text-foreground font-medium truncate max-w-[180px]">{item.note}</span> : "Add note"}
                                  </button>
                                )}
                              </div>
                            </div>
                      ...
                      {recommendations.length > 0 && (
                        <div className="pt-1 pb-1">
                          <RecommendationBar items={recommendations} label="You might also like" />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex-shrink-0 border-t border-border px-4 pt-3 pb-5 space-y-3">
                      ...
                      <motion.div whileTap={{ scale: 0.98 }}>
                        <Button className="w-full h-12 text-base font-semibold gap-2" onClick={() => goTo("checkout", 1)} data-testid="button-proceed-checkout">
              ...
              <motion.div key="checkout" custom={direction} variants={slideVariants}
                initial={direction > 0 ? "enterRight" : "enterLeft"} animate="center"
              ...
              >
                <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
                  <div className="space-y-1.5">
                    ...
                    <Input placeholder="e.g. Rahul Sharma" value={customerName} onChange={(e) => setCustomerName(e.target.value)} data-testid="input-customer-name" />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Textarea placeholder="Any special requests for the kitchen..." value={orderNotes}
                      onChange={(e) => setOrderNotes(e.target.value)} rows={2} className="resize-none" data-testid="input-order-notes" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      <PaymentOption value="online" selected={paymentMethod === "online"} onClick={() => setPaymentMethod("online")}
                        icon={<Smartphone className="h-5 w-5" />} label="Pay Online" description="UPI, Cards, Wallets via Razorpay" />
                      <PaymentOption value="cod" selected={paymentMethod === "cod"} onClick={() => setPaymentMethod("cod")}
                        icon={<Banknote className="h-5 w-5" />} label="Pay at Counter" description="Cash or Card at the billing counter" />
                    </div>
                  </div>
                  ...
                  <div className="bg-muted/50 rounded-xl p-3.5 space-y-2">
                    ...
                    <div className="space-y-1 text-sm">
                      ...
                      <div className="flex justify-between text-muted-foreground"><span>GST (5%)</span><span>₹{tax.toFixed(0)}</span></div>
                      <div className="flex justify-between font-bold text-base">
                        <span>Total</span>
                ...
                <div className="flex-shrink-0 border-t border-border px-4 pt-3 pb-5">
                  <motion.div whileTap={{ scale: 0.98 }}>
                    <Button className="w-full h-12 text-base font-semibold gap-2" onClick={handlePlaceOrder}
                      disabled={createOrder.isPending} data-testid="button-place-order">
                      {createOrder.isPending ? "Placing Order..." :
                        paymentMethod === "online" ? <><Wallet className="h-5 w-5" /> Pay ₹{total.toFixed(0)} Online</> :
                        <><Banknote className="h-5 w-5" /> Place Order — Pay at Counter</>}
                    </Button>
                  </motion.div>
                  <p className="text-center text-xs text-muted-foreground mt-2">
                    {paymentMethod === "online" ? "You'll be redirected to Razorpay after placing" : "Pay cash or card when your food arrives"}
                  </p>
                </div>
              </motion.div>
            )}
